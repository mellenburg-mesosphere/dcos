""" Utilites for interacting with a DC/OS instance via REST API

Most DC/OS instances will be with auth enabled
"""
import copy
import logging
import os
...
from test_util.helpers import ApiClientSession, lazy_property, path_join, Url
...
class DcosUser:
    ...
    def authenticate(self, dcos_url: str):
        logging.info('Attempting authentication')
        # explicitly use a session with no user authentication for requesting auth headers
        r = requests.post(path_join(dcos_url, '/acs/api/v1/auth/login'), json=self.credentials, auth=None)
        r.raise_for_status()
        ...
        # Set requests auth
        self.auth = DcosAuth(self.auth_token)
...
class ARNodeApiClientMixin:
    def api_request(self, method, path_extension, scheme=None, host=None, query=None,
        ...
        """ Communicating with a DC/OS cluster is done by default through Adminrouter.
        Use this Mixin with an ApiClientSession that requires distinguishing between nodes.
        Adminrouter has both a master and agent process and so this wrapper accepts a
        node argument. node must be a host in self.master or self.all_slaves. If given,
        the request will be made to the adminrouter endpoint for that node type
        """
        assert self.masters
        ...
        if node is not None:
            ...
            if node in self.masters:
                # Nothing else to do, master adminrouter uses default HTTP (80) and HTTPS (443) ports
                pass
...
class DcosApiSession(ARNodeApiClientMixin, ApiClientSession):
    def __init__(self, dcos_url: str, masters: list, public_masters: list,
                 slaves: str, public_slaves: list, default_os_user: str,
        ...
        # waiting must be performed with init to guarantee that spin-off API's have
        # this session's auth settings
        self.wait_for_dcos()
    ...
    def _authenticate_default_user(self):
        ...
        if self.auth_user is not None:
            self.auth_user.authenticate(str(self.default_url))
            self.session.auth = self.auth_user.auth
    ...
    def copy(self):
        """ Copying allows splitting off Client sessions that use this client's auth
        Session implicitly has cookies set, so explicitly remove them when making a copy
        """
    ...
    def get_user_session(self, user):
        ...
        new.auth_user = None
        if user is not None:
            user.authenticate(str(new.default_url))
